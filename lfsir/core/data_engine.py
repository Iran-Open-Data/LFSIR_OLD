...
from . import metadata_reader, decoder
...
def extract_dependencies(table_name: str, year: int) -> dict:
    ...
    while len(table_list) > 0:
        ...
        if table in original_tables:
            ...
            local_path = defaults.cleaned_data.joinpath(file_name)
            size = local_path.stat().st_size if local_path.exists() else None
            dependencies[table] = {"size": size}
        elif "table_list" in metadata.schema[table]:
...
class TableHandler:
    ...
    def _create_table(self, table_name: _OriginalTable) -> pd.DataFrame:
        table = open_and_clean_table(table_name=table_name, year=self.year)
...
class TableFactory:
    ...
    def __init__(
        ...
        self.year = year
        self.settings = settings if settings is not None else LoadTableSettings()
        schema = utils.resolve_metadata(metadata.schema, year)
