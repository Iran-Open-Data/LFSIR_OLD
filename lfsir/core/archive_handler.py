...
from .metadata_reader import defaults, _Years
from .. import utils
...
def _get_zip_file_name(year: int) -> str:
    if year in range(1396, 1400):
...
def _download_year_file(year: int, replace: bool = True) -> None:
    ...
    file_url = f"{defaults.online_dir}/original_files/{_get_zip_file_name(year)}"
...
def _unpack_yearly_data_archive(year: int, replace: bool = True):
    ...
    """
    file_path = defaults.archive_files.joinpath(_get_zip_file_name(year))
